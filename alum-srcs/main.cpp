...
bool
   ...
   modoVBO = false, //modo visualizacion
...
void FGE_PulsarTeclaCaracter( GLFWwindow* window, unsigned int codepoint )
{
   ...
   switch (toupper(ch))
   {
      ...
      case 'M' :
         ...
         break ;
      case 'V':
         if(contextoVis.modoVBO == false){
           contextoVis.modoVBO = true;
           cout << "modo de visualización cambiado a: modo diferido" << endl << flush ;
         }
         else{
           contextoVis.modoVBO = false;
           cout << "modo de visualización cambiado a: modo inmediato" << endl << flush ;
         }
         break;
      default:
         redibujar = false;
...
void Inicializa_Vars( )
{
   ...
   practicaActual = 1 ;
   contextoVis.modoVis = modoAlambre ; //para que no haya que pulsar tecla en el inicio
