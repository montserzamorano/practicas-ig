...
void P5_Inicializar(  int vp_ancho, int vp_alto )
{
   ...
   camaras[2] = new CamaraInteractiva(false, radio, 0, -90, {0.0,0.0,0.0}, false,80.0,2);
   camActiva = 0;
...
void P5_ClickIzquierdo( int x, int y )
{
   ...
   if(id==0){
     ...
     if(objetoActivo5->buscarObjeto(id, m, &objBuscado,centroObj)){
       cout << "Identificador del objeto: " << id << endl;
       cout << "Seleccionado objeto " << objBuscado->leerNombre() << endl;
       camaras[camActiva]->modoExaminar(centroObj);
     }
     else{
